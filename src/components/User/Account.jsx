  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: "8px" }}>
        {user.boxes.length < 1 ? (
          <Typography>
            Du hast noch keine senseBox registriert. Besuche{" "}
            <Link
              color="primary"
              rel="noreferrer"
              target="_blank"
            ...
            um eine senseBox zu registrieren.
          </Typography>
        ) : (
          <Typography style={{ fontWeight: "bold", fontSize: "1.1rem" }}>
            Du hast {user.boxes.length}{" "}
            {user.boxes.length === 1 ? "senseBox" : "senseBoxen"} registriert:
          </Typography>
        )}
      </div>
      ...
      <Grid container spacing={2}>
        {user.boxes.map((box, i) => {
          const sensors = box.sensors.map((sensor) => sensor.title);
          return (
