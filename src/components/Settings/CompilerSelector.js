...
class CompilerSelector extends Component {
  ...
  render(){
    return (
      <div>
        <Typography style={{fontWeight: 'bold'}}>{Blockly.Msg.settings_compiler}</Typography>
        <FormHelperText style={{color: 'black', lineHeight: 1.3, marginBottom: '8px'}}>{Blockly.Msg.settings_compiler_text}</FormHelperText>
        ...
        />
      <FormControlLabel control={<Checkbox defaultChecked />} checked={this.state.readOnly} onChange={this.changeStatus} label="Change URL" />
        </FormControl>
      </div>
    );
  }
