  ...
  const config = {
    success: (res) => {
      ...
      if (project) {
        ...
        dispatch({ type: PROJECT_PROGRESS });
        dispatch(
          returnSuccess(res.data.message, res.status, "GET_PROJECT_SUCCESS"),
        );
      } else {
        dispatch({ type: PROJECT_PROGRESS });
...
export const updateProject = (type, id) => (dispatch, getState) => {
  ...
  const config = {
    success: (res) => {
      ...
      if (type === "project") {
        dispatch(
          returnSuccess(res.data.message, res.status, "PROJECT_UPDATE_SUCCESS"),
        );
      } else {
        dispatch(
          returnSuccess(res.data.message, res.status, "GALLERY_UPDATE_SUCCESS"),
        );
      }
    },
    error: (err) => {
...
export const deleteProject = (type, id) => (dispatch, getState) => {
  const config = {
    success: (res) => {
      ...
      if (type === "project") {
        dispatch(
          returnSuccess(res.data.message, res.status, "PROJECT_DELETE_SUCCESS"),
        );
      } else {
        dispatch(
          returnSuccess(res.data.message, res.status, "GALLERY_DELETE_SUCCESS"),
        );
      }
    },
    error: (err) => {
...
export const shareProject = (title, type, id) => (dispatch, getState) => {
  ...
  axios
    ...
    .then((res) => {
      ...
      dispatch(
        returnSuccess(res.data.message, shareContent._id, "SHARE_SUCCESS"),
      );
    })
    .catch((err) => {
      if (err.response) {
